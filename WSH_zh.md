...
| `case ... esac` | 解析器未实现 |
| `&&` 操作符 | 分词器未实现 |
| `||` 操作符 | 看似可用（被管道分割偶然触发），但非真正的短路逻辑 |
| `break` / `continue` | 未实现 |
| `$#`、`$0`-`$9` | 位置参数不可用 |
| `$!` | 后台进程 PID 不适用 |
| 引号剥离 | 单引号和双引号会原样传递给命令 |
| `${VAR:-default}` | 变量修饰符未实现 |
| `${#VAR}` | 字符串长度未实现 |
| `echo -e` 在管道中 | `\n` 在 wsh 管道数据中不会展开 |
| 交互模式 | 仅支持 `-c CMD` 模式 |
...
- **引号处理**：`echo 'hello'` 输出 `'hello'`（带引号）。这是已知 bug——变量展开时未剥离引号。变通方案是尽量避免使用引号：
  ```bash
  echo hello world          # 正常工作（简单词不需要引号）
  echo $VAR                 # 变量展开无需引号
  ```
- **多行管道数据**：包含换行的数据，先写入文件，再从文件管道读取。
- **`||` 行为**：`false || echo fallback` 看似可用，但这是意外行为——`||` 被管道 `|` 分割器切分，不是真正的 `||` 操作符。请勿依赖此行为。
